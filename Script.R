  ...
  conditionalPanel(
    ...
    tabsetPanel(
      ...
      tabPanel("Machine Parameters",
               ...
               numericInput("T_warm","Warm-up time (h):",0.5),
               numericInput("T_cool","Cool-down time (h):",0.5),
      ...
      tabPanel("ML Estimation",
               sliderInput("n_neurons","Number of neurons:",1,10,5),
...
server <- function(input, output, session) {
  observeEvent(input$calc, {
    ...
    C_tri_med <- C_unit; C_tri_hi <- C_unit*(1+delta)
    PI <- if(C_star<=C_tri_lo)1 else if(C_star>=C_tri_hi)0 else 1-(C_star-C_tri_lo)/(C_tri_hi-C_tri_lo)
  ...
  observeEvent(input$train, {
    ...
    nn <- neuralnet(
      y~rho+V_s+V_j+HS+H,
      data=df_norm,
      hidden=c(input$n_neurons,input$n_neurons,input$n_neurons),
      linear.output=TRUE
    )
    plot(nn)
    ...
    output$histogram<- renderPlot({hist(err,breaks=30,main="Error distribution",xlab="Error (s)")})
    output$scatter  <- renderPlot({plot(df_raw$y,pred,pch=19,xlab="True build time (s)",ylab="Predicted build time (s)");abline(0,1)})
  })
}
